...
Page({
  ...
  questionsGet: function(){
    ...
    //获取问题列表
    wx.request({
      url: app.url.host + app.url.answerHistoriesUrl,
      method: 'GET',
      data: {
        isRight: -1,
      },
    ...
    wx.request({
      ...
      success: function(res){
        if(res.statusCode == '200') {
          ...
          that.progressHandler(progressData);

          //答题卡需要从progress里获取状态
          that.addCardList(questionsData);        
        } else {
          app.unauthorized(res.statusCode);
        } //statusCode-else结束
      }
  ...
  progressHandler: function(progressData) {
    ...
    if (progressData) {
      ...
      console.log('无进度信息，初始化');
      //不能获取为第一次做题，初始化状态，post
      var questionsStatus = {};
    ...
    //修改当前题号
    this.changeCurNum(this.data.questionsStatus);
  },
