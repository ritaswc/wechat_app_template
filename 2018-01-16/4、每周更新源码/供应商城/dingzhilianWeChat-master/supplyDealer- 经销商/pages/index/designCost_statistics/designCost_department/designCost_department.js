...
Page({
  ...
  onLoad: function (option) {
    ...
    that.designForDeptOrManageer(that);
  },
  goDetail: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/index/designCost_statistics/designCost_detail/designCost_detail?dept_id=' + dept_id + '&m_id=' + m_id + '&p_id=' + p_id + '&start=' + start + '&end=' + end + '&dept=' + dept + '&manageer=' + manageer + '&principal=' + principal
    })
  },
  designForDeptOrManageer: function (that) {
    var that = this, adminObj = app.globalData.adminObj;
    wx.request({
      url: app.globalData.requestUrl + "weixinMerchant/designStatisticForDeptOrManageer",
      data: {
        id: that.data.id,
